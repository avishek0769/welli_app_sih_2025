...
# Streamlit UI
def main():
    st.set_page_config(page_title="Mental Health Scale Estimator", layout="wide")
    st.title("Mental Health Scale Estimator")

    user_msgs = load_conversation_json()
    if not user_msgs:
        st.warning("No conversation.json found. Please chat first.")
        return

    if len(user_msgs) < 3:
        st.info("Need at least 3 user messages to estimate scales.")
        return
    ...
    c1.metric("PHQ-9", f"{scores['PHQ-9']['total']}", scores['PHQ-9']['level'])
    c2.metric("GAD-7", f"{scores['GAD-7']['total']}", scores['GAD-7']['level'])
    c3.metric("GHQ-12", f"{scores['GHQ-12_binary']['total']}", scores['GHQ-12_binary']['level'])
    c4.metric("Overall Risk", scores['overall_risk'])
    ...
    st.subheader("Recent User Conversation")
